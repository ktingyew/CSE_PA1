...
char *path = "/mnt/c/Users/kting/Desktop//PA1/logfile_test.txt";
...
static int create_daemon()
{
	...
	pid_t child1 = fork();
	if (child1 < 0){
		printf("Fork unsuccessful. Intermediate process not created.\n");
		exit(1);
	}
	...
	id_t child2 = fork();
	if (child2 < 0){
		printf("Fork unsuccessful. Daemon not created.\n");
		exit(1);
	}
	if (child2 > 0){ // what to do with the intermediate process?
		exit(1);
	}
